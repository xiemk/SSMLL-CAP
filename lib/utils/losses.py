...
def loss_an(logits, observed_labels):
    ...
    corrected_loss_matrix = F.binary_cross_entropy_with_logits(logits, torch.logical_not(observed_labels).type(logits.type()), reduction='none')
...
def compute_batch_loss(preds, label_vec, args): # "preds" are actually logits (not sigmoid activated !)
