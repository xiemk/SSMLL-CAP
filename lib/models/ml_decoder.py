...
class MLDecoder(nn.Module):
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=768,
                 initial_num_features=2048, proj_dim = 128,  zsl=0):
    ...
    def forward(self, x, le=False):  # label embedding
        ...
        embedding_spatial_768 = self.decoder.embed_standart(embedding_spatial)
        embedding_spatial_768 = torch.nn.functional.relu(embedding_spatial_768, inplace=True)
        ...
        h = self.decoder(tgt, embedding_spatial_768.transpose(0, 1))  # [embed_len_decoder, batch, 768]
        h = h.transpose(0, 1)
